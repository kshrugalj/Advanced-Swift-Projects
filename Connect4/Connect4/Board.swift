...
func diagonalVictory() -> Bool {
    for column in 0...board.count {
        if checkDiagnonalColumn(column, true, true) {
            return true
        }
        if checkDiagnonalColumn(column, true, false) {
            return true
        }
        if checkDiagnonalColumn(column, false, true) {
            return true
        }
        if checkDiagnonalColumn(column, false, false) {
            return true
        }
    }
    return false
...
func checkDiagnonalColumn(_ columnToCheck: Int, _ moveUp: Bool, _ reverseRows: Bool) -> Bool {
    var movingColumn = columnToCheck
    var consecutive = 0
    
    let rowSequence = reverseRows ? board.reversed() : board
